...
const StorePage = () => {
  const handleBundleOrder = (bundleType) => {
    // Implement logic for handling bundle order here
    console.log(`Ordered €{bundleType} Bundle!`);
  };

  const navigation = useNavigation(); // Use the useNavigation hook here

  const handleGoBack = () => {
    navigation.goBack(); // Navigate back to the previous screen (TermsAndConditionsScreen)
  };
        ...
        <View style={styles.bundlesContainer}>
          <TouchableOpacity style={styles.bundle} onPress={() => handleBundleOrder('Basic')}>
            ...
            <Text style={styles.bundleDescription}>
              A simple bundle with essential </Text>
              <Text style={styles.bundleDescription}>
                 27 Pictures.</Text>
                 <Text style={styles.bundleDescription}>
             + free Delivery
